...
export const mapRawUnitToUnit = (raw: RawUnit): Unit => ({
	...
	section: str(raw['Секция']),
	salesStartDate: str(raw['Старт продаж']),
	salesEndDate: str(raw['Окончание продаж']),
	commissioningDate: str(raw['Ввод в эксплуатацию']),
	unitNumber: str(raw['№ помещения']),
	floor: str(raw['Этаж']),
	roomsCount: str(raw['Число комнат']),
	totalAreaSqm: str(raw['Общая площадь, кв. м']),
	livingAreaSqm: str(raw['Жилая площадь, кв. м']),
	...
	actualStatus: str(raw['Статус факт']),
	actualPricePerSqmRub: str(raw['Цена 1 кв. м факт, ₽']),
	actualTotalPriceRub: str(raw['Общая цена факт, ₽']),
